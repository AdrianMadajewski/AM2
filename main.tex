    ...
    \begin{equation*}
        \begin{split}
            F(b) - F(a) &= \sum_{j=1}^{n}(F(x_j) - F(x_{j-1})) \\
                        &= \sum_{j=1}^{n}f(\xi_j)(x_j - x_{j-1}) \leq U(f, P) < \varepsilon + \int_{a}^{b}f(x)dx, \\
            F(b) - F(a) &\geq L(f, P) > -\varepsilon + \int_{a}^{b}f(x)dx.            
        \end{split}
    \end{equation*}
    Stąd
    \begin{equation*}
...
\begin{proof}
    Stosując Wzór Newtona-Leibniza i wzór na całkowanie przez częsci wykonujemy następujący ciąg przekształceń, w którym wszystkie różniczkowania i podstawienia
