...
% Customize footer
\fancyfoot[C]{\thepage}
\fancyfoot[R]{\textcopyright{} Adrian Madajewski 2024}
...
\[
\int_{a}^{b}f(x)dx - \frac{\varepsilon}{2} \leq L(f,P) \leq U(f, P) \leq \int_{a}^{b}f(x)dx + \frac{\varepsilon}{2}
\]
...
\[
0 \leq \overline{\int_{a}^{b}f(x)dx} - \underline{\int_{a}^{b}f(x)dx} < \varepsilon
\]
Z dowolności $\varepsilon > 0$ wynika, że $\underline{\int_{a}^{b}f(x)dx} = \overline{\int_{a}^{b}f(x)dx}$. Oznaczając ponadto $\underline{\int_{a}^{b}f(x)dx} = \overline{\int_{a}^{b}f(x)dx} = I$ mamy $L(f, P) \leq I \leq U(f, P)$.
...
\[
    U(f, P) \leq U(f, P_2) < \overline{\int_{a}^{b}fdx} + \frac{\varepsilon}{2} = \underline{\int_{a}^{b}fdx} + \frac{\varepsilon}{2} < L(f, P_1) + \varepsilon \leq L(f, P) + \varepsilon
\]
Stąd $U(f, P) - L(f, P) \leq \varepsilon$, a zatem warunek (\ref{eq:38}) jest spełniony. Wobec Twierdzenia $\ref{theorem:76}$ dowód jest zakończony.
...
dla którego $\delta(P) < \delta$. Wtedy mamy $M_i - m_i \leq \frac{\varepsilon}{b-a}$ dla $i = 1,\ldots,n$ i wobec tego
\[
    U(f, P) - L(f, P) = \sum_{i=1}^{n}(M_i - m_i)\Delta x_i \leq \frac{\varepsilon}{b-a}\sum_{i=1}^{n}\Delta x_i = \varepsilon
\]
...
\begin{proof}
    ...
    Ponadto
    \[
        \sum_{i \in A}(M_i - m_i)\Delta x_i \leq \frac{\varepsilon}{2(b-a)}\sum_{i \in A}\Delta x_i \leq \frac{\varepsilon}{2(b-a)}(b-a)=\frac{\varepsilon}{2}
    \]
    ...
    Dlatego
    \[
        \sum_{i \in B}(M_i - m_i)\Delta x_i \leq (M-m)\sum_{i \in B}\Delta x_i < (M-m)\frac{\varepsilon}{2(M-m)}=\frac{\varepsilon}{2}
    \]
...
\begin{proof}
    ...
    $M_i^* - m_i^* < \varepsilon$ dla $i \in A$. Natomiast dla $i \in B$ mamy $M_i^* - m_i^* \leq 2K$, gdzie $K=\sup{\{|\phi(t)|:m \leq t \leq M\}}$.
    Stąd otrzymujemy
    \[
    ...
    a zatem
    \[
        U(h, P) - L(h, P) \leq \varepsilon(a + b + 2K)
    \]
...
\]
\[
    \forall_{\varepsilon > 0} \exists_{\delta > 0} \forall_P \forall_{R \in \mathfrak{R}(f+g, P)} \delta(P) < \delta \implies |R - I| \leq |R_f - I_1| + |R_g - I_2| < \varepsilon
\]
...
\begin{theorem} 
{
    (a) Jeśli funkcje $f$ i $g$ są R-całkowalne na przedziale $[a,b]$ oraz $f(x) \leq g(x)$ dla każdego $x \in [a,b]$, to
    \[
...
\end{theorem}
\begin{proof}
    (a) Jeśli $m \leq f(x) \leq M$ dla $x \in [a,b]$, to
    \[
        m(b-a) \leq \int_{a}^{b}f(x)dx \leq M(b-a)
    \]
    Stąd, jeśli $f(x) \geq 0$ dla $x \in [a,b]$, to $\int_{a}^{b}f(x)dx \geq 0$.
    Wobec tego nierówność $f(x) \leq g(x)$ dla $x \in [a,b]$ implikuje
    \[
        \int_{a}^{b}f(x)dx \leq \int_{a}^{b}g(x)dx
    \]
    (b) Biorąc $\phi(t) = |t|$ w Twierdzeniu $\ref{theorem:81}$ otrzymujemy całkowalność funkcji $|f|$.
    Ponieważ $-|f(x)| \leq f(x) \leq |f(x)|$ dla $x \in [a,b]$, zatem na mocy (a) otrzymujemy
    \[
        \left\vert\int_{a}^{b}f(x)dx\right\vert \leq \int_{a}^{b}|f(x)|dx
    \]
\end{proof}
